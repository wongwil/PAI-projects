...
class Model(object):
    ...
    def __init__(self):
        ...
        self.model = GaussianProcessRegressor(kernel=Matern(nu=0.5), random_state=0, n_restarts_optimizer=5)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        mask = (test_x_AREA == 1)
        predictions = np.array(gp_mean, copy=True)  
        predictions[mask] += gp_std[mask]*7
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        X_train, _, y_train, _ = train_test_split(train_x_2D, train_y, test_size=0.2, random_state=70, shuffle=True)
        
        sample_size = 7000
        small_x = X_train[:sample_size]
        small_y = y_train[:sample_size]
        self.model.fit(small_x, small_y)
