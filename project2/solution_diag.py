...
class SWAGInference(object):
    ...
    def __init__(
        ...
        swag_epochs: int = 2, # TODO change back to 30
        swag_learning_rate: float = 0.045,
        swag_update_freq: int = 1,
        deviation_matrix_max_rank: int = 2,
        bma_samples: int = 2, # TODO change back to 30
    ):
        """
    ...
    def update_swag(self, n=None) -> None:
        ...
        for name, param in current_params.items():
            ...
            # SWAG paper, 3.3.
            self.swag_mean_diagonal[name] = (n*self.swag_mean_diagonal[name] +  param) / (n+1) # theta_SWA (mean of all \theta)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        self.D_matrix = self._create_weight_copy()
        for name in self.D_matrix.keys():
            self.D_matrix[name] = collections.deque() # create D for each parameter
